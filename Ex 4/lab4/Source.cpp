...
void addList(List2W& l1, List2W& l2) {
	...
	while (p2 != sen2) {
		...
		while (temp->elem.key >= p1->next->elem.key && p1->next != sen1)
			p1 = p1->next;
		temp->next = p1->next;
		temp->prev = p1;
		temp->prev->next = temp;
...
void deleteDoubled (List2W& l) {
	ElementLL *sen = l.sentinel;
	if (sen->next != sen) {
		ElementLL *p = sen->next;
		while (p->next!=sen) {
			if (p->next->elem.key == p->elem.key) {
				ElementLL *temp = p->next;
				p->next = temp->next;
				temp->next->prev = p;
				delete temp;
			}
...
int longestSeqDifferent(List2W& l) {
	ElementLL *sen = l.sentinel;
	if (sen->next != sen) {
		int seq = 1;
		int max_seq=1;
		...
		while (p->next!=sen) {
			if (p->elem.key < p->next->elem.key)
				seq++;
			else if (p->elem.key > p->next->elem.key && seq > max_seq) {
				max_seq = seq;
				seq = 1;
			}
			p = p->next;
			if (p->next == sen && seq > max_seq) {
				max_seq = seq;
				return max_seq;
			}
		}
...
int main() {
	...
	while (true) {
		...
		if (isCommand(command, "DD"))
		{
			deleteDoubled(list[currentL]);
			continue;
		}

		if (isCommand(command, "LS"))
		{
			cout << longestSeqDifferent(list[currentL]) << endl;;
			continue;
		}
